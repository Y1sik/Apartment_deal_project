<template>
  <div class="modal" tabindex="-1" id="boardNoticeUpdateModal">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <input
              v-model="$store.state.notice.title"
              type="text"
              class="form-control"
              placeholder="제목"
            />
          ...
          <div class="mb-3">
            <div id="divEditorNoticeUpdate"></div>
          </div>
          <!-- 기존 파일 내용 보여줌  -->
          ...
          <div class="form-check mb-3">
            <input
              ...
              type="checkbox"
              value=""
              id="chkFileUploadNoticeUpdate"
            />
            <label class="form-check-label" for="chkFileUploadNoticeUpdate"
              >파일 추가</label
            >
          </div>
          <div
            class="mb-3"
            v-show="attachFile"
            id="imgFileUploadNoticeUpdateWrapper"
          >
            <input
              @change="changeFile"
              type="file"
              id="inputFileUploadNoticeUpdate"
              multiple
            />
            <div
              id="imgFileUploadNoticeUpdateThumbnail"
...
export default {
  ...
  methods: {
    ...
    noticeUpdate() {
      ...
      formData.append("noticeId", this.$store.state.notice.noticeId); // update 에 추가
      formData.append("title", this.$store.state.notice.title);
      formData.append("content", this.CKEditor.getData()); // store X !!!!

      // file upload
      var attachFiles = document.querySelector("#inputFileUploadNoticeUpdate");
      console.log("NoticeUpdateModalVue: data 1 : ");
      ...
      http
        .post(
          "/notices/" + this.$store.state.notice.noticeId, // store boardId
          formData,
          { headers: { "Content-Type": "multipart/form-data" } }
        )
        .then(({ data }) => {
          console.log("NoticeUpdateModalVue: data : ");
          console.log(data);
        ...
        .catch((error) => {
          console.log("NoticeUpdateModalVue: error ");
          console.log(error);
        });
    },
    closeModal() {
      this.$emit("call-parent-notice-update"); // no parameter
    },
  },
  // modal.show() 이전에 이미 mounted() 호출됨
  mounted() {
    ClassicEditor.create(document.querySelector("#divEditorNoticeUpdate"))
      .then((editor) => {
    ...
    this.$el.addEventListener("show.bs.modal", function () {
      ...
      this.fileList = [];
      document.querySelector("#inputFileUploadNoticeUpdate").value = "";
    });
  },
  // watch 사용 X
  // props 사용 X
