    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <input
              v-model="$store.state.free.title"
              type="text"
              class="form-control"
              placeholder="제목"
            />
          ...
          <div v-if="$store.state.free.fileList.length > 0" class="mb-3">
            첨부파일 :
            <span
              ><div
...
export default {
  ...
  methods: {
    ...
    freeUpdate() {
      ...
      formData.append("freeId", this.$store.state.free.freeId); // update 에 추가
      formData.append("title", this.$store.state.free.title);
      ...
      http
        .post(
          "/frees/" + this.$store.state.free.freeId, // store boardId
          formData,
  ...
  mounted() {
    ...
    this.$el.addEventListener("show.bs.modal", function () {
      $this.CKEditor.setData($this.$store.state.free.content);
      // 첨부 파일 관련 초기화
